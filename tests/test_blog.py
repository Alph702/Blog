...
def test_admin_login_logout(page: Page, flask_app_url):
    ...
    expect(page).to_have_title("Login - Blog", timeout=300000)
    ...
    expect(page.locator("a", has_text="New Post")).to_be_visible(timeout=300000)
    expect(page.locator("a", has_text="Logout")).to_be_visible(timeout=300000)

    page.locator("a", has_text="Logout").click()
    expect(page).to_have_url(f"{flask_app_url}/", timeout=300000)
    expect(page.locator("a", has_text="Login")).to_be_visible(timeout=300000)
    expect(page.locator("a", has_text="New Post")).not_to_be_visible(timeout=300000)
...
def test_admin_login_incorrect_credentials(page: Page, flask_app_url):
    ...
    expect(page.locator(".error-message")).to_have_text("Invalid credentials", timeout=300000)
...
def test_delete_post_authorization(page: Page, flask_app_url):
    ...
    page.goto(f"{flask_app_url}/delete/1", timeout=300000)
    expect(page).to_have_url(f"{flask_app_url}/login", timeout=300000)

def test_admin_inspect_authorization(page: Page, flask_app_url):
    response = page.goto(f"{flask_app_url}/admin/inspect", timeout=300000)
    expect(page.locator("body")).to_have_text("{\"error\":\"admin only\"}", timeout=300000)
    assert response.status == 403

def test_create_and_view_post(admin_logged_in_page: Page, flask_app_url):
    page = admin_logged_in_page
    page.goto(f"{flask_app_url}/new", timeout=300000)
    expect(page).to_have_title("New Post", timeout=300000)
    ...
    expect(page.locator("h1", has_text=test_title)).to_be_visible(timeout=300000)

    page.locator("a", has_text=test_title).click()
    post_id = page.url.split('/')[-1]

    expect(page.locator("h1")).to_have_text(test_title, timeout=300000)
    expect(page.locator("p").nth(1)).to_have_text(test_content, timeout=300000)
...
def test_create_post_with_image(admin_logged_in_page: Page, flask_app_url):
    ...
    page.locator("a", has_text=test_title).click()
    post_id = page.url.split('/')[-1]
    expect(page.locator(".image")).to_be_visible(timeout=300000)
...
def test_delete_post(admin_logged_in_page: Page, flask_app_url):
    ...
    expect(page).to_have_url(f"{flask_app_url}/", timeout=300000)
    expect(page.locator("h1", has_text=test_title)).not_to_be_visible(timeout=300000)
...
def test_filter_posts_ui(admin_logged_in_page: Page, flask_app_url):
    ...
    expect(filter_panel).not_to_have_class(re.compile("open"), timeout=300000)
    page.click("#toggleFilterBtn")
    expect(filter_panel).to_have_class(re.compile("open"), timeout=300000)

    page.select_option("#year", "any")
    page.select_option("#month", "any")
    page.select_option("#day", "any")
    page.click("button[type='submit']")
    expect(page).to_have_url(re.compile(f"{flask_app_url}/filter.*year=any.*month=any.*day=any"), timeout=300000)
