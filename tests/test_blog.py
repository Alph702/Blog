...
def test_create_and_view_post(admin_logged_in_page: Page, flask_app_url):
    ...
    page.goto(f"{flask_app_url}/new", timeout=600000)
    expect(page).to_have_title("New Post", timeout=600000)
...
def test_filter_posts_ui(admin_logged_in_page: Page, flask_app_url):
    ...
    expect(filter_panel).not_to_have_class(re.compile("open"), timeout=600000)
    page.click("#toggleFilterBtn")
    expect(filter_panel).to_have_class(re.compile("open"), timeout=600000)
