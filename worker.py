import os, subprocess, supabase, uuid
...
class Worker:
    ...
    def save_file(self, file):
        ...
        file.save(filepath)
        self.supabase_client.table('files').insert({'filename': filename, 'filepath': filepath}).execute()
        file_id = self.supabase_client.table('files').select('id').eq('filepath', filepath).execute().data[0]['id']
    ...
    def process_file(self, file_id):
        result = self.supabase_client.table('files').select('filepath', 'filename').eq('id', file_id).execute()
        if not result.data:
        ...
        try:
            self.supabase_client.table('files').update({'status': 'processing'}).eq('id', file_id).execute()
            subprocess.run(cmd, check=True)
            self.supabase_client.table('files').update({'status': 'processed', 'filepath': os.path.join(out_dir, "master.m3u8")}).eq('id', file_id).execute()
        except subprocess.CalledProcessError as e:
            self.supabase_client.table('files').update({'status': 'failed'}).eq('id', file_id).execute()
